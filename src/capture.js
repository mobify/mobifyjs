...
var removeCloseEndTags = function(html) {
    ...
    return removeCloseEndTags(newHtml);
}
...
Capture.initStreamingCapture = function(chunkCallback, finishedCallback, options) {
    ...
    var pollPlaintext = function(){
        ...
        if (capturedDoc.documentElement) {
            ...
            // TODO: With the `removeCloseEndTags` method, the line below can be removed
            var endIndex = (capturedHtml.lastIndexOf('</body></html>') === -1) ? capturedHtml.lastIndexOf('</head></html>') : capturedHtml.lastIndexOf('</body></html>'); 
            var toWriteDest = capturedHtml.substring(writtenToDestDoc.length, endIndex);

            // outerHTML will always give us an balanced tree, which isn't what
            // what to write into the destination document. The solution for
            ...
            toWriteDest = removeCloseEndTags(toWriteDest);
